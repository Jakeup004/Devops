...
const codecoveragefactory_1 = require("azure-pipelines-tasks-codecoverage-tools/codecoveragefactory");
const CodeAnalysisOrchestrator_1 = require("azure-pipelines-tasks-codeanalysis-common/Common/CodeAnalysisOrchestrator");
const BuildOutput_1 = require("azure-pipelines-tasks-codeanalysis-common/Common/BuildOutput");
const CheckstyleTool_1 = require("azure-pipelines-tasks-codeanalysis-common/Common/CheckstyleTool");
const PmdTool_1 = require("azure-pipelines-tasks-codeanalysis-common/Common/PmdTool");
const FindbugsTool_1 = require("azure-pipelines-tasks-codeanalysis-common/Common/FindbugsTool");
const javacommons = require("azure-pipelines-tasks-java-common/java-common");
const jsdom = require("jsdom");
const { JSDOM } = jsdom;
const util = require("./utils/mavenutil");
const spotbugsTool = require("./spotbugsTool");
...
var failIfCoverageEmptySetting = tl.getBoolInput('failIfCoverageEmpty');
const restoreOriginalPomXml = tl.getBoolInput('restoreOriginalPomXml');
const isSpotbugsAnalysisEnabled = tl.getBoolInput('spotBugsAnalysisEnabled', false);
const spotBugsGoal = tl.getInput('spotBugsGoal');
const isFailWhenBugsFoundBySpotbugs = tl.getBoolInput('spotBugsFailWhenBugsFound', false);
var codeCoverageFailed = false;
var summaryFile = null;
var reportDirectory = null;
var reportPOMFile = null;
var ccReportDir = null;
let buildOutput = new BuildOutput_1.BuildOutput(tl.getVariable('System.DefaultWorkingDirectory'), BuildOutput_1.BuildEngine.Maven);
var codeAnalysisOrchestrator = new CodeAnalysisOrchestrator_1.CodeAnalysisOrchestrator([new CheckstyleTool_1.CheckstyleTool(buildOutput, 'checkstyleAnalysisEnabled'),
...
function execBuild() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        ccReportDir = yield execEnableCodeCoverage();
        var userRunFailed = false;
        var codeAnalysisFailed = false;
        ...
        configureMavenOpts();
        // 1. Check that Maven exists by executing it to retrieve its version.
        let settingsXmlFile = null;
        yield mvnGetVersion.exec()
            .fail(function (err) {
            ...
            if (authenticateFeed) {
                ...
                return repositories
                    ...
                    let options = mavenOptions ? mavenOptions.match(/([^" ]*("([^"\\]*(\\.[^"\\]*)*)")[^" ]*)|[^" ]+/g) : undefined;
                    if (options) {
                        mavenOptions = '';
                        for (let i = 0; i < options.length; ++i) {
                            if ((options[i] === '--settings' || options[i] === '-s') && (i + 1) < options.length) {
                                i++; // increment to the file name
                                let suppliedSettingsXml = path.resolve(tl.cwd(), options[i]);
            ...
            return __awaiter(this, void 0, void 0, function* () {
                ...
                if (isSpotbugsAnalysisEnabled) {
                    yield spotbugsTool.AddSpotbugsPlugin(mavenPOMFile);
...
function applySonarQubeArgs(mvnsq) {
    const isJacocoCoverageReportXML = tl.getBoolInput('isJacocoCoverageReportXML', false);
...
function configureMavenOpts() {
    let mavenOptsValue = tl.getInput('mavenOpts');
    if (mavenOptsValue) {
...
function publishJUnitTestResults(testResultsFiles) {
    var matchingJUnitResultFiles = undefined;
...
function execEnableCodeCoverage() {
...
function enableCodeCoverage() {
    ...
    let ccEnabler = new codecoveragefactory_1.CodeCoverageEnablerFactory().getTool("maven", ccTool.toLowerCase());
    return ccEnabler.enableCodeCoverage(buildProps);
}
function publishCodeCoverage(isCodeCoverageOpted) {
    var defer = Q.defer();
    if (isCodeCoverageOpted) {
...
let currentPlugin = '';
function processCurrentPluginFromOutput(data) {
...
function processSpotbugsOutput(data) {
...
function processMavenOutput(buffer) {
    if (buffer == null) {
        return;
    }
...
*/
function readCodeCoverageReportAsDom(directory) {
    const htmlString = fs.readFileSync(path.join(directory, 'index.html'), 'utf-8');
 ...
 * @param fileName - name of html file with extension
*/
function writeDomAsHtml(directory, dom, fileName) {
...
function replaceImageSourceToBase64(dir) {
